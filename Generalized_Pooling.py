...
def get_attention_weights(inputs, num_da, scope, scope_reuse=False):
...
def fc_layer(inputs, output_size, dropout_keep_prob, scope, scope_reuse=False):
    with tf.variable_scope(scope, reuse=scope_reuse):
...
class GeneralizedPooling(object):
    def __init__(self, max_len_left, max_len_right, vocab_size,
                embedding_size, num_stack, rnn_size, num_heads, num_da,
                 num_hidden, penalty_type=0, mu=1e-3, l2_reg_lambda=0.0):
        ...
        with tf.name_scope("embedding"):
            W_weight = tf.get_variable(shape=[vocab_size, embedding_size],
                                        dtype=tf.float32,
                                        initializer=tf.truncated_normal_initializer())
            self.embedding_weight = tf.concat([tf.constant(tf.zeros([1, embedding_size])), W_weight[1:]],
                                              axis=1, name='embedding_weights')
        ...
        with tf.name_scope('generalized_pooling'):
            ...
            for i in range(num_heads):
                ...
                A_i_left, w_1  = get_attention_weights(self.H_left, num_da, "head_{}".format(i), scope_reuse=False)
                A_i_left = get_masked_weights(A_i_left, self.length_left, max_len_left)

                A_i_right, _   = get_attention_weights(self.H_right, num_da, "head_{}".format(i), scope_reuse=True)
            ...
            if penalty_type == 0:
                for i in range(num_heads):
                    for j in range(i+1, num_heads):
                        self.penalty += tf.relu(1 - tf.square(tf.norm(W_1[i]-W_1[j], ord='fro', axis=[0,1])))
        ...
        with tf.name_scope('mlp_layer'):
            ...
            output = fc_layer(self.V, num_hidden, self.dropout_keep_prob, 'fc_1', scope_reuse=False)
            # has a shortcut connection
            self.full_put = fc_layer(tf.concat([self.V, output], axis=-1), num_hidden, self.dropout_keep_prob, 'fc_2', scope_reuse=False)
